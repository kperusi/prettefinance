    ...
    if (singleIncome?.main === "Income") {
      try {
        ...
        await deleteDoc(doc(db, "Income", id))
          .then(() => {
            ...
            const updatedIncomes = JSON.parse(localStorage.getItem(`${account_types} incomes`));
            const updatedIncomesAfterDelete = updatedIncomes.filter(
              (income) => income.id !== id
            );
            localStorage.setItem(`${account_types} incomes`,JSON.stringify(updatedIncomesAfterDelete))
      ...
      try {
        ...
        await deleteDoc(doc(db, "Expenses", id))
          .then(() => {
            const updatedIncomes = JSON.parse(localStorage.getItem(`${account_types}expenses`));
            const updatedIncomesAfterDelete = updatedIncomes.filter(
              (expenses) => expenses.id !== id
            );
             localStorage.setItem(`${account_types} expenses`,JSON.stringify(updatedIncomesAfterDelete))
            // localStorage.setItem(
            //   "expenses",
            //   JSON.stringify(updatedIncomesAfterDelete)
            // );
      ...
      <section className="single-income-section-two">
        <div className="single-income-amount">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <p style={{fontSize:'1.7em'}}>{Splitter(singleIncome?.amount)}</p>
          </div>
        </div>
        ...
        <div className="single-income-desc-cx">
          ...
          <div className="single-income-desc">
            <p style={{color:'grey', fontSize:'1em'}}>Reference No.:</p>
            <p style={{fontSize:'1.2em'}}>{singleIncome?.id}</p>
          </div>
          <hr />
          <div>
            <p  style={{color:'grey', fontSize:'1em'}}>Date:</p>
            <p style={{fontSize:'1.2em'}}>{singleIncome?.date}</p>
          </div>
          <hr />

          {singleIncome?.incomeSource && (
            <div>
              <p style={{color:'grey' ,fontSize:'1em'}}>Income From: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.incomeSource}</p>
              <hr />
            </div>
          )}
          ...
          {singleIncome?.expensesCategory && (
            <div>
              <p  style={{color:'grey' ,fontSize:'1em'}}>Given to: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.expensesCategory}</p>
              <hr />
            </div>
          )}
          ...
          {singleIncome?.MOD && (
            <div>
              <p style={{color:'grey', fontSize:'1em'}}>Means of Funding: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.MOD}</p>
              <hr />
            </div>
          )}
          ...
          {singleIncome?.desc && (
            <div>
              <p  style={{color:'grey', fontSize:'1em'}}>Description: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.desc}</p>
              <hr />
            </div>
          )}
          ...
          {singleIncome?.givenBy && (
            <div>
              <p  style={{color:'grey' ,fontSize:'1em'}}>Given By: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.givenBy}</p>
              <hr />
            </div>
          )}
          ...
          {singleIncome?.createdBy && (
            <div>
              <p  style={{color:'grey', fontSize:'1em'}}>Transaction Initiated By: </p>
              <p style={{fontSize:'1.2em'}}>{singleIncome?.createdBy}</p>
              <hr />
            </div>
          )}
        </div>
